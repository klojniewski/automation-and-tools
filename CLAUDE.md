...
- `followup <idOrUrl>` — interactive: produce a draft follow-up email for a single deal
  ...
  - Flow:
    ...
    11. Asks whether to log the follow-up as a Pipedrive **activity** on the deal (default Yes) — creates a **"Follow Up"**-type activity (`createDealActivity`; type key resolved from the label via `getActivityTypeKey`, falls back to `task`), subject `Follow-up: <subject>`, note = email body, due today, **marked done**, linked to the primary contact
...
- `src/lib/pipedrive.ts` — `createDealActivity` (logs the drafted follow-up as an activity on the deal, marked done, best-effort; called from step 11) + `getActivityTypeKey` (resolves an activity-type label like "Follow Up" to its `key_string`, cached per-process)
